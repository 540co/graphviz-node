...
class Graph {
  ...
  addEdge(nodeOne, nodeTwo, attributes) {
    ...
    let edge = new Edge(_nodeOne, _nodeTwo);
  ...
  toDot() {
    ...
    if (this._id) {
      ...
      dotScript = this._type + " {\n";
    }

    // Graph attributes
    if (this._graphAttributes.length() > 0) {
      dotScript = dotScript + "  graph" + this._graphAttributes.toDot() + ";\n";
    }

    // Node attributes
    if (this._nodesAttributes.length() > 0) {
      dotScript = dotScript + "  node" + this._nodesAttributes.toDot() + ";\n";
    }

    // Edge attributes
    if (this._edgesAttributes.length() > 0) {
      dotScript = dotScript + "  edge" + this._edgesAttributes.toDot() + ";\n";
    }

    // Each node
    for (let [key, value] of Object.entries(this._nodes)) {
      if (this._nodes.hasOwnProperty(key)) {
        dotScript = dotScript + "  " + this._nodes[key].toDot() + ";\n";
      }
    }

    // Each HTML node
    for (let [key, value] of Object.entries(this._htmlNodes)) {
      if (this._htmlNodes.hasOwnProperty(key)) {
        dotScript = dotScript + "  " + this._htmlNodes[key].toDot() + ";\n";
      }
    }

    // Each edge
    for (let i = 0; i < this._edges.length; i++) {
      dotScript = dotScript + "  " + this._edges[i].toDot() + ";\n";
    }
